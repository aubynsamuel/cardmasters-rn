...
var GameUtils_1 = require("../src/gameLogic/GameUtils");
exports.GAME_TO = 10;
var MultiplayerCardsGame = /** @class */ (function () {
  ...
  MultiplayerCardsGame.prototype.handleGameState = function () {
    ...
    var _a = (0, GameUtils_1.dealCards)(this.players, this.deck),
      hands = _a.hands,
      deck = _a.deck;
    this.deck = deck;
    var updatedPlayers = this.players.map(function (player, idx) {
      return __assign(__assign({}, player), { hands: hands[idx] });
    });
   ...
   */
  MultiplayerCardsGame.prototype.playerPlayCard = function (
    playerId,
    card,
    index
  ) {
    ...
    if (
      this.currentPlays.some(function (play) {
        return play.player.id === playerId;
      })
    ) {
    ...
    if (this.currentLeadCard) {
      ...
      var hasRequired = this.players[playerIndex].hands.some(function (c) {
        return c.suit === requiredSuit_1;
      });
    ...
    updatedPlayers[playerIndex] = __assign(
      __assign({}, updatedPlayers[playerIndex]),
      { hands: newHand }
    );
  ...
  MultiplayerCardsGame.prototype.playCard = function (player, card) {
    var newPlays = __spreadArray(
      __spreadArray([], this.currentPlays, true),
      [{ player: player, card: card }],
      false
    );
    var newLeadCard =
      this.currentPlays.length === 0 ? card : this.currentLeadCard;
    var newHistory = __spreadArray(
      __spreadArray([], this.gameHistory, true),
      [
        {
          message: ""
            .concat(player.name, " played ")
            .concat(card.rank)
            .concat(GameUtils_1.suitSymbols[card.suit]),
          importance: false,
        },
      ],
      false
  ...
  MultiplayerCardsGame.prototype.finishRound = function () {
    ...
    var newHistory = __spreadArray(
      ...
      [
        {
          message: ""
            .concat(newControl.name, " Won Round ")
            .concat(this.cardsPlayed + 1),
          importance: true,
        },
      ],
      false
    ...
    var winnerIndex = this.players.findIndex(function (p) {
      return p.id === winningPlayer.id;
    });
    this.updateState({
  ...
  MultiplayerCardsGame.prototype.handleGameOver = function (
    newControl,
    ...
    var winnerIndex = updatedPlayers.findIndex(function (p) {
      return p.id === newControl.id;
    });
    updatedPlayers[winnerIndex] = __assign(
      __assign({}, updatedPlayers[winnerIndex]),
      { score: updatedPlayers[winnerIndex].score + finalPoints }
    );
    this.updateState({
      players: updatedPlayers,
      message: "\uD83C\uDFC6 "
        .concat(newControl.name, " won this game with ")
        .concat(finalPoints, " points! \uD83C\uDFC6"),
    });
    // Check if any player has reached the winning score.
    var gameWinner = updatedPlayers.find(function (p) {
      return p.score >= exports.GAME_TO;
    });
    if (!gameWinner) {
      setTimeout(function () {
        _this.startGame();
      ...
      var scores = updatedPlayers.map(function (p) {
        return {
          playerName: p.name,
          score: p.score,
        };
  ...
  MultiplayerCardsGame.prototype.resetGame = function () {
    var resetPlayers = this.players.map(function (player) {
      return __assign(__assign({}, player), { score: 0, hands: [] });
    });
